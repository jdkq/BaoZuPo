...
public class PhotoDao {
    ...
    *这个函数的功能是将文件传到预先设置的绝对路径中，也就是项目里的img文件夹
     * @throws IOException 
     */
    public static boolean saveFile(FileItem item,String fileName) throws IOException {        
        File savePath1=new File("G:\\Java\\java\\ChuZuPo\\WebContent\\img");
        if(!savePath1.exists()) {
            savePath1.mkdirs();
        }
     
        File uploadFile1=new File(savePath1 + File.separator+fileName);
        try{
            item.write(uploadFile1);
            return true;
        }catch(Exception e){
        }
        return false;
